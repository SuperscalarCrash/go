...
func TestSizesForLoong32r(t *testing.T) {
	for _, compiler := range []string{"gc", "gccgo"} {
		sizes := types2.SizesFor(compiler, "loong32r")
		if sizes == nil {
			t.Fatalf("SizesFor(%q, %q) = nil", compiler, "loong32r")
		}
		if got := sizes.Sizeof(types2.Typ[types2.Uintptr]); got != 4 {
			t.Errorf("SizesFor(%q, %q).Sizeof(uintptr) = %d, want 4", compiler, "loong32r", got)
		}
	}

	gccgo := types2.SizesFor("gccgo", "loong32r")
	if got := gccgo.Alignof(types2.Typ[types2.Float64]); got != 8 {

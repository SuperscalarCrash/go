...
var regNamesLOONG32R = []string{
	...
	"R28",
	"R29", // closure context
	// R30 is REGTMP and is not used by regalloc
	"R31",
...
func init() {
	...
	if len(regNamesLOONG32R) > 64 {
		panic("too many registers")
	}
	num := map[string]int{}
	for i, name := range regNamesLOONG32R {
		num[name] = i
	}
	...
	var (
		gp         = buildReg("R4 R5 R6 R7 R8 R9 R10 R11 R12 R13 R14 R15 R16 R17 R18 R19 R20 R21 R23 R24 R25 R26 R27 R28 R29 R31")
		gpg        = gp | buildReg("g")
		gpsp       = gp | buildReg("SP")
		...
		first4     = buildReg("R4 R5 R6 R7")
	)
	// Common regInfo
	var (
		gp01      = regInfo{inputs: nil, outputs: []regMask{gp}}
		gp11      = regInfo{inputs: []regMask{gpg}, outputs: []regMask{gp}}
		gp11same  = regInfo{inputs: []regMask{gp}, outputs: []regMask{gp}}
		gp11sp    = regInfo{inputs: []regMask{gpspg}, outputs: []regMask{gp}}
		gp21      = regInfo{inputs: []regMask{gpg, gpg}, outputs: []regMask{gp}}
		gp31      = regInfo{inputs: []regMask{gp, gp, gp}, outputs: []regMask{gp}}
		gp2pair   = regInfo{inputs: []regMask{gpg, gpg}, outputs: []regMask{gp, gp}}
	...
	ops := []opData{
		...
		{name: "MUL", argLength: 2, reg: gp21, asm: "MUL", commutative: true},                                         // low 32 bits of arg0 * arg1
		{name: "MULT", argLength: 2, reg: gp2pair, commutative: true, typ: "(Int32,Int32)", resultNotInArgs: true},    // high, low words of signed product
		{name: "MULTU", argLength: 2, reg: gp2pair, commutative: true, typ: "(UInt32,UInt32)", resultNotInArgs: true}, // high, low words of unsigned product
		{name: "DIV", argLength: 2, reg: gp2pair, typ: "(Int32,Int32)", resultNotInArgs: true},                        // remainder, quotient
		{name: "DIVU", argLength: 2, reg: gp2pair, typ: "(UInt32,UInt32)", resultNotInArgs: true},                     // remainder, quotient
		...
		// LA32R has no count-leading-zero instruction. These two pseudo-ops
		// are expanded by the compiler into base integer instructions.
		{name: "CTZ", argLength: 1, reg: gp11same, resultInArg0: true, needIntTemp: true},
		{name: "BITLEN", argLength: 1, reg: gp11same, resultInArg0: true, needIntTemp: true},
		...
		{name: "CMOVZ", argLength: 3, reg: gp31, resultInArg0: true},
		{name: "CMOVZzero", argLength: 2, reg: regInfo{inputs: []regMask{gp, gpg}, outputs: []regMask{gp}}, resultInArg0: true},
		...
		//	BNE	Rarg2, R1, -4(PC)
		{
			...
			argLength: 4,
			reg: regInfo{
				inputs:   []regMask{buildReg("R20"), buildReg("R21"), gp},
		...
		// Scheduler ensures LoweredGetClosurePtr occurs only in the entry block
		// and sorts it to the beginning of that block, before R29 can be reused.
		...
		// The call clobbers R1 (the architectural link register), R30
		// (the linker temporary), and all allocatable registers except g.
		// Returns a pointer to a write barrier buffer in R25.
		{name: "LoweredWB", argLength: 1, reg: regInfo{clobbers: callerSave &^ buildReg("g R25"), outputs: []regMask{buildReg("R25")}}, clobberFlags: true, aux: "Int64"},
	...
	archs = append(archs, arch{
		name:            "LOONG32R",
		pkg:             "cmd/internal/obj/loong32r",
		genfile:         "../../loong32r/ssa.go",
		ops:             ops,
		blocks:          blocks,
		regnames:        regNamesLOONG32R,
		gpregmask:       gp,
		fpregmask:       fp,
		specialregmask:  0,

...
func progedit(ctxt *obj.Link, p *obj.Prog, newprog obj.ProgAlloc) {
	// The Plan 9 parser treats BREAK's sole immediate as a destination.
	// LA32R encodes it in the same unsigned 15-bit field as SYSCALL, whose
	// immediate is parsed as a source, so normalize the two forms here.
	if p.As == ABREAK && p.From.Type == obj.TYPE_NONE && p.To.Type == obj.TYPE_CONST {
		p.From = p.To
		p.To = obj.Addr{}
	}
...
func rewriteToUseGot(ctxt *obj.Link, p *obj.Prog, newprog obj.ProgAlloc) {
	...
	var source *obj.Addr
	// MOVx sym, Ry becomes MOVW sym@GOT, REGTMP; MOVx (REGTMP), Ry
	// MOVx Ry, sym becomes MOVW sym@GOT, REGTMP; MOVx Ry, (REGTMP)
...
func (c *ctxt0) stacksplit(p *obj.Prog, framesize int32) *obj.Prog {
	...
	if c.ctxt.Flag_maymorestack != "" {
		// Save LR and REGCTXT in a frame that preserves the LA32R
		// 16-byte stack alignment at the call to maymorestack.
		frameSize := int64(16)
		...
		p.As = mov
		p.From.Type = obj.TYPE_MEM
		p.From.Offset = frameSize - int64(c.ctxt.Arch.PtrSize)
		p.From.Reg = REGSP
		p.To.Type = obj.TYPE_REG
		p.To.Reg = REGCTXT
...
var Linkloong32r = obj.LinkArch{
	Arch:           sys.ArchLoong32r,
	Init:           buildop,
	Preprocess:     preprocess,
	Assemble:       span0,
	Progedit:       progedit,
	DWARFRegisters: LOONG32RDWARFRegisters,
}
